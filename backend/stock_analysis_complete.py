...
def predict_stock_price(symbol: str, horizon: str = "intraday", verbose: bool = True):
    ...
    try:
        snapshot_columns = [
            ...
            'macd_hist_increasing', 'SMA_10', 'SMA_20', 'SMA_50', 'SMA_200',
            'ADX', 'BB_lower', 'BB_middle', 'BB_upper', 'BB_width', 'BB_pct',
            'bb_position', 'ATR', 'volatility_20', 'volume_ratio',
            'volume_trend', 'trend_direction', 'higher_high', 'lower_low',
            'price_to_sma_10', 'price_to_sma_50', 'price_to_sma_200',
            'daily_return', 'daily_return_ma_5'
        ]
        latest_row = latest_features.iloc[0] if hasattr(latest_features, 'iloc') else latest_features
        for column in snapshot_columns:
            if column in latest_row:
                value = latest_row[column]
                try:
                    feature_snapshot[column] = round(float(value), 6)
                except (TypeError, ValueError):
                    feature_snapshot[column] = value
    except Exception as snapshot_error:
        logger.warning(f"Could not build feature snapshot for {symbol}: {snapshot_error}")
