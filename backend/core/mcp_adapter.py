...
from market_intelligence import enrich_prediction, enrich_predictions_response
...
class MCPAdapter:
        ...
        try:
            ...
            for symbol in symbols:
                try:
                    ...
                    prediction = predict_stock_price(symbol, horizon=horizon, verbose=False)
            ...
            predictions = [enrich_prediction(p) for p in predictions]
    ...
    def scan_all(
        ...
        try:
            ...
            if isinstance(response.get("shortlist"), list):
                response["shortlist"] = [enrich_prediction(p) for p in response["shortlist"]]
            if isinstance(response.get("all_predictions"), list):
                response["all_predictions"] = [enrich_prediction(p) for p in response["all_predictions"]]
    ...
    def analyze(
        ...
        try:
            ...
            duration_ms = (time.time() - start_time) * 1000
            response = enrich_predictions_response(response)
            self._log_response(request_id, response, duration_ms)
