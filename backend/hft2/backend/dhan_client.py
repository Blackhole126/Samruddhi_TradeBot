...
_marketfeed_unsubscribed_tokens: Dict[str, float] = {}
_MARKETFEED_UNSUBSCRIBED_TTL: float = 3600.0


def _is_marketfeed_subscription_error(error_data: Any) -> bool:
    """Dhan returns 806 when trading APIs work but quote APIs are not subscribed."""
    if not isinstance(error_data, dict):
        return False
    data = error_data.get("data")
    if isinstance(data, dict):
        return any(str(code) == "806" or "Data APIs not Subscribed" in str(msg)
                   for code, msg in data.items())
    return "Data APIs not Subscribed" in json.dumps(error_data, default=str)
...
def _dhan_client_id_from_jwt(token: Optional[str]) -> Optional[str]:
    try:
        ...
        client_id = data.get("dhanClientId") or data.get("dhan_client_id")
        return str(client_id).strip() if client_id else None
    except Exception:
        return None


def _normalize_dhan_credentials(client_id: Optional[str], access_token: Optional[str]) -> tuple[str, str]:
    cid = (client_id or "").strip()
    token = (access_token or "").strip()

    if _looks_like_jwt(cid) and not _looks_like_jwt(token):
        logger.warning("Detected swapped Dhan credentials; normalizing client_id/access_token.")
        return token, cid

    if _looks_like_jwt(cid):
        extracted_cid = _dhan_client_id_from_jwt(cid)
        if extracted_cid:
            logger.warning("Detected JWT passed as Dhan client_id; using dhanClientId from token payload.")
...
def _dhan_request(
    ...
    client_id_header: str = "dhanClientId",
    **kwargs: Any,
) -> Any:
    ...
    if client_id:
        headers[client_id_header] = str(client_id)

    # Log request details for debugging (mask sensitive parts)
    logger.debug(f"Dhan API Request: {method} {url}")
    logger.debug(
        f"Headers: access-token={'***' + token[-4:] if token else 'None'}, {client_id_header}={client_id}")
        ...
        try:
            ...
            if body:
                try:
                    ...
                    if path == "/marketfeed/ltp" and _is_marketfeed_subscription_error(error_data):
                        logger.warning(
                            "Dhan Market Feed LTP unavailable: Data APIs not subscribed. "
                            "Portfolio connection is still valid; using price fallbacks."
                        )
                        return {
                            "status": "failure",
                            "http_code": e.code,
                            "error": error_data,
                            "market_data_unsubscribed": True,
                        }
            ...
            try:
                return {"status": "failure", "http_code": e.code, "error": error_data or json.loads(body)}
            except Exception:
...
def _get_dhan_ltps(token: str, instruments: List[Dict], client_id: Optional[str] = None) -> Dict[str, float]:
    ...
    Dhan market quote v2 expects a client-id header and a body grouped by
    exchange segment, e.g. {"NSE_EQ": [11536]}.
    """
    if not instruments:
        return {}
    if token and (time.time() - _marketfeed_unsubscribed_tokens.get(token, 0)) < _MARKETFEED_UNSUBSCRIBED_TTL:
        logger.debug("Skipping Dhan Market Feed LTP: quote API subscription unavailable for this token.")
        return {}

    body: Dict[str, List[int]] = {}
    def _marketfeed_segment(raw: str) -> str:
    ...
    for item in instruments:
        segment = _marketfeed_segment(str(item.get("exchangeSegment") or ""))
        security_id = str(item.get("securityId") or "").strip()
        ...
        try:
            ...
            continue
        body.setdefault(segment, []).append(sid)
    if not body:
        return {}
    ...
    try:
        ...
        if isinstance(out, dict):
            if out.get("market_data_unsubscribed"):
                if token:
                    _marketfeed_unsubscribed_tokens[token] = time.time()
                return {}
            data = out.get("data") if isinstance(out.get("data"), dict) else out
            for segment_data in data.values():
                if not isinstance(segment_data, dict):
                    continue
                for sid, quote in segment_data.items():
                    if not isinstance(quote, dict):
                        continue
                    price = float(quote.get("last_price") or quote.get("lastPrice") or 0.0)
                    if price > 0:
                        prices[str(sid)] = price
        if isinstance(out, list):
            for item in out:
...
def get_live_portfolio(access_token: Optional[str] = None, client_id: Optional[str] = None) -> Optional[Dict]:
    ...
    cid, token = _normalize_dhan_credentials(client_id, access_token)
    ...
    if instruments_to_fetch:
        ...
        dhan_prices = _get_dhan_ltps(
            token, instruments_to_fetch, client_id=cid)
        for h in holdings.values():
...
class DhanAPIClient:
    ...
    def __init__(self, client_id: str, access_token: str):
        _load_env()
        self.client_id, self.access_token = _normalize_dhan_credentials(client_id, access_token)
