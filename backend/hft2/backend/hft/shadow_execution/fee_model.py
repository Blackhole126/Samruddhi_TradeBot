...
class FeeModel:
    """
    Backward-compatible single-leg fee model used by the shadow simulator tests.
    FeeImpactCalculator remains the richer round-trip calculator.
    """
    ...
    def calculate_fees(
        ...
        turnover = max(float(price), 0.0) * max(int(qty), 0)
        side = (side or "BUY").upper()
        is_buy = side == "BUY"
        is_intraday = trade_type == TradeType.EQUITY_INTRADAY
        is_delivery = trade_type == TradeType.EQUITY_DELIVERY

        brokerage = min(turnover * FeeImpactCalculator.BROKERAGE_PCT,
                        FeeImpactCalculator.BROKERAGE_MAX)
        brokerage = max(brokerage, FeeImpactCalculator.BROKERAGE_MIN) if turnover > 0 else 0.0

        if is_intraday:
            stt = turnover * FeeImpactCalculator.STT_INTRADAY_SELL_PCT if not is_buy else 0.0
            stamp_duty = turnover * FeeImpactCalculator.STAMP_DUTY_INTRADAY_PCT if is_buy else 0.0
            tax_category = "BUSINESS_INCOME"
        elif is_delivery:
            stt = turnover * FeeImpactCalculator.STT_DELIVERY_BOTH_PCT
            stamp_duty = turnover * FeeImpactCalculator.STAMP_DUTY_DELIVERY_PCT if is_buy else 0.0
            tax_category = "STCG_OR_LTCG"
        elif trade_type == TradeType.CRYPTO_SPOT:
            stt = 0.0
            stamp_duty = 0.0
            tax_category = "VDA_FLAT_TAX"
        else:
            stt = turnover * (
                FeeImpactCalculator.STT_FUTURES_SELL_PCT if not is_buy else 0.0
            )
            stamp_duty = turnover * FeeImpactCalculator.STAMP_DUTY_FUTURES_PCT if is_buy else 0.0
            tax_category = "BUSINESS_INCOME"

        exchange_txn_charge = turnover * (
            FeeImpactCalculator.EXCHANGE_TXN_CRYPTO_PCT
            if trade_type == TradeType.CRYPTO_SPOT
            else FeeImpactCalculator.EXCHANGE_TXN_NSE_PCT
        )
        sebi_fees = turnover * FeeImpactCalculator.SEBI_TURNOVER_PCT
        gst = (brokerage + exchange_txn_charge + sebi_fees) * FeeImpactCalculator.GST_PCT
        total = brokerage + stt + exchange_txn_charge + sebi_fees + stamp_duty + gst

        return FeeBreakdown(
            brokerage=round(brokerage, 4),
            stt=round(stt, 4),
            exchange_txn_charge=round(exchange_txn_charge, 4),
            sebi_fees=round(sebi_fees, 4),
            stamp_duty=round(stamp_duty, 4),
            gst=round(gst, 4),
            total_tax_and_fees=round(total, 4),
