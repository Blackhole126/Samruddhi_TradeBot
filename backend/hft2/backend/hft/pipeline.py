...
from .risk import RiskGate, RegimeThrottler, VolatilityRegime
...
class HFTPipeline:
    ...
    def process_tick(self, tick: Tick) -> None:
        ...
        current_vol = 0.0 
        event_time = datetime.fromtimestamp(tick.timestamp)
        market_regime = self.regime_detector.update(event_time, tick.symbol, tick.price, current_vol)
        ...
        feature_values = {
            ...
            "regime_confidence": market_regime.confidence if market_regime else 0.0,
            "estimated_fee_impact_bps": 0.0,
        }
        features = FeatureVector(
            timestamp=event_time,
            symbol=tick.symbol,
            feature_hash=FeatureVector.compute_hash(feature_values),
            **feature_values,
        )
        ...
        if market_regime:
             regime_map = {
                 "HIGH_VOLATILITY": VolatilityRegime.HIGH,
                 "EXTREME_VOLATILITY": VolatilityRegime.EXTREME,
             }
             self.simulator.set_regime(regime_map.get(market_regime.regime.value, VolatilityRegime.NORMAL))
