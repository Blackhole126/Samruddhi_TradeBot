...
class ExecutionTool:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # Validate that LiveTradingExecutor is provided
        if "live_executor" not in config:
            raise ValueError(
                "LiveTradingExecutor must be provided in ExecutionTool config")
        ...
        logger.info(
            f"Execution Tool {self.tool_id} initialized in LIVE mode (delegates to LiveTradingExecutor)")
    ...
    async def execute_trade(self, arguments: Dict[str, Any], session_id: str) -> MCPToolResult:
        ...
        try:
            ...
            self.total_volume += execution_result.filled_quantity * \
            ...
            if order_id:
                ...
                if order_id in self.active_orders:
                    ...
                    historical_order = next(
                        (o for o in self.order_history if o.order_id == order_id), None)
                    if historical_order:
                        response_data = {
                ...
                if include_history:
                    response_data["order_history"] = [
                        # Last 20 orders
                        asdict(order) for order in self.order_history[-20:]]
    ...
    async def _perform_risk_checks(self, order: TradeOrder, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Order value check
            estimated_value = order.quantity * \
                (order.price or 2500)  # Use price or default
    ...
    async def _execute_live_order(self, order: TradeOrder, arguments: Dict[str, Any]) -> TradeOrder:
        """Bridge MCP orders to the configured LiveTradingExecutor."""
        live_executor = self.config.get("live_executor")
        if live_executor is None:
            raise ValueError("LiveTradingExecutor is not configured")
        ...
        if not isinstance(result, dict):
            raise ValueError("LiveTradingExecutor returned an invalid response")

        order.updated_at = datetime.now()
        if result.get("success"):
            order.status = OrderStatus.FILLED
            order.filled_quantity = int(result.get("quantity") or order.quantity)
            order.filled_price = float(result.get("price") or order.price or arguments.get("price") or 0.0)
        ...
        order.status = OrderStatus.REJECTED
        message = result.get("message") or result.get("error") or "Live order rejected"
        raise ValueError(message)

    async def _update_portfolio(self, order: TradeOrder) -> Dict[str, Any]:
        """Update portfolio after order execution - delegated to LiveTradingExecutor/DB"""
        try:
            if order.status != OrderStatus.FILLED:
                return {"updated": False, "reason": "Order not filled"}

            # Portfolio is updated by LiveTradingExecutor in database
            ...
            portfolio_impact = {
                ...
                "timestamp": datetime.now().isoformat(),
                "source": "LiveTradingExecutor/Database"
            }
        ...
        try:
            ...
            try:
                today_trades = session.query(Trade).filter(
                    Trade.timestamp >= datetime.combine(
            ...
            logger.error(f"Failed to calculate daily P&L from database: {e}")
            return 0.0
