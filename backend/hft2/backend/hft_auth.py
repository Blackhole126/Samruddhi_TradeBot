...
def _normalize_demat_credentials(client_id: Optional[str], access_token: Optional[str]) -> tuple[str, str]:
    cid = (client_id or "").strip()
    token = (access_token or "").strip()

    if _looks_like_jwt(cid) and not _looks_like_jwt(token):
        logger.warning("Detected swapped Dhan credentials in storage; normalizing in memory.")
        return token, cid
...
def get_user_demat(username: str) -> Optional[dict]:
    ...
    client_id, token = _normalize_demat_credentials(client_id, token)
