...
class LivePriceValidator:
    ...
    def get_live_price_data(self, symbol: str) -> Dict:
        ...
        future = executor.submit(self._fetch_live_price_data, symbol)
        try:
            return future.result(timeout=self.fetch_timeout_seconds)
        except TimeoutError:
            logger.error(f"Timed out getting live price for {symbol}")
            future.cancel()
            return self._cached_price_fallback(symbol, "Live price fetch timed out")
        except Exception as e:
            logger.error(f"Failed to get live price for {symbol}: {e}")
            return self._cached_price_fallback(symbol, str(e))
        finally:
            executor.shutdown(wait=False, cancel_futures=True)

    def _fetch_live_price_data(self, symbol: str) -> Dict:
        try:
    ...
    def _cached_price_fallback(self, symbol: str, reason: str) -> Dict:
        cache_path = Path(__file__).resolve().parent / "data" / "cache" / f"{symbol}_all_data.json"
        feature_path = Path(__file__).resolve().parent / "data" / "features" / f"{symbol}_features.json"

        for path in (cache_path, feature_path):
            if not path.exists():
                continue
            try:
                ...
                cached_price = (
                    payload.get("info", {}).get("currentPrice")
                    or payload.get("current_price")
                    or payload.get("current_features", {}).get("Close")
                )
                if not cached_price:
                    continue
                ...
                fetch_time = (
                    ...
                    or payload.get("metadata", {}).get("fetch_timestamp")
                    or payload.get("latest_date")
                )
                return {
                    'symbol': symbol,
                    'current_price': float(cached_price),
                    'price_timestamp': str(fetch_time or datetime.now().isoformat()),
                    'price_source': 'cached_fallback',
                    'exchange': payload.get("info", {}).get("exchange", "UNKNOWN"),
                    'currency': payload.get("info", {}).get("currency", "UNKNOWN"),
                    'market_state': 'UNKNOWN',
                    'is_delayed': True,
                    'delay_minutes': None,
                    'regular_market_price': cached_price,
                    'regular_market_time': None,
                    'previous_close': payload.get("info", {}).get("previousClose"),
                    'day_change': None,
                    'day_change_percent': None,
                    'day_high': None,
                    'day_low': None,
                    'volume': payload.get("current_features", {}).get("Volume"),
                    'fetch_timestamp': datetime.now().isoformat(),
                    'data_freshness': 'CACHED_FALLBACK',
                    'fallback_reason': reason,
                    'validation': {
                        'is_valid': True,
                        'warnings': [f"Live source unavailable: {reason}", "Using cached fallback data"],
                        'errors': []
                    }
                }
            except Exception as cache_error:
                logger.warning(f"Cached fallback failed for {symbol} at {path}: {cache_error}")
