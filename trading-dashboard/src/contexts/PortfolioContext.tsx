...
export const PortfolioProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const loadPortfolioFromStorage = async (portfolioId: PortfolioType) => {
    try {
      ...
      // Validate we have a user context before proceeding
      if (!storage || !user?.username) {
        console.warn('[PORTFOLIO CONTEXT] No valid user context, initializing empty portfolio');
        setHoldings([]);
        setTotalValue(0);
        setTotalGain(0);
        setTotalGainPercent(0);
      ...
      const savedHoldings = storage.getItem(key);

      console.log(`[PORTFOLIO CONTEXT] Loading ${portfolioId} portfolio for user: ${user.username}`);

      if (savedHoldings) {
        try {
          const parsed = JSON.parse(savedHoldings);
          setHoldings(parsed);
          recalculatePortfolioMetrics(parsed);
          setLastUpdated(new Date());
          console.log(`[PORTFOLIO CONTEXT] Loaded ${parsed.length} holdings from storage`);
        } catch (parseError) {
          console.error('[PORTFOLIO CONTEXT] Failed to parse saved holdings:', parseError);
          setHoldings([]);
          setTotalValue(0);
          setTotalGain(0);
          setTotalGainPercent(0);
          setLastUpdated(new Date());
          storage.removeItem(key); // Clear corrupted data
        }
      } else {
        console.log(`[PORTFOLIO CONTEXT] No saved data found for ${portfolioId}, starting empty`);
        setHoldings([]);
        setTotalValue(0);
        setTotalGain(0);
        setTotalGainPercent(0);
  ...
  const savePortfolioToStorage = (portfolioId: PortfolioType, holdingsToSave: PortfolioHolding[]) => {
    try {
      const storage = getUserStorageScoped();
      if (!storage || !user?.username) {
        console.warn('[PORTFOLIO CONTEXT] Cannot save - no valid user context');
        return;
      }

      const key = `portfolio_${portfolioId}_holdings`;
      storage.setItem(key, JSON.stringify(holdingsToSave));
      console.log(`[PORTFOLIO CONTEXT] Saved ${holdingsToSave.length} holdings for user ${user.username}`);
    } catch (err) {
