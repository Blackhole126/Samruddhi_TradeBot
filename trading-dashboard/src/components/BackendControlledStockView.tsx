...
function MarketIntelligencePanel({ prediction }: ModelOutputPanelProps) {
  const intelligence = prediction.market_intelligence;

  if (!intelligence) {
    return null;
  }

  const context = intelligence.market_context || {};
  const patterns = intelligence.evolving_patterns || {};
  const strength = intelligence.strength_building || [];
  const weakness = intelligence.weakness_forming || [];
  const risk = intelligence.risk_increasing || [];

  return (
    <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
      <h4 className="font-medium text-gray-900 mb-2">SAMRUDDHI INTELLIGENCE</h4>
      {intelligence.plain_language_summary && (
        <p className="text-gray-800 text-xs mb-3">{intelligence.plain_language_summary}</p>
      )}
      <div className="grid grid-cols-1 gap-2 text-xs">
        <div className="flex justify-between gap-3">
          <span className="text-gray-600">Context:</span>
          <span className="text-gray-900 text-right">{formatSignalLabel(context.regime || context.market_context)}</span>
        </div>
        <div className="flex justify-between gap-3">
          <span className="text-gray-600">Momentum:</span>
          <span className="text-gray-900 text-right">{formatSignalLabel(patterns.momentum_shift)}</span>
        </div>
        <div className="flex justify-between gap-3">
          <span className="text-gray-600">Breakout:</span>
          <span className="text-gray-900 text-right">
            {formatSignalLabel(patterns.breakout_state)} / {formatSignalLabel(patterns.breakout_quality)}
          </span>
        </div>
        <div className="flex justify-between gap-3">
          <span className="text-gray-600">Continuation:</span>
          <span className="text-gray-900 text-right">{formatSignalLabel(patterns.continuation_vs_exhaustion)}</span>
        </div>
      </div>

      {(strength.length > 0 || weakness.length > 0 || risk.length > 0) && (
        <div className="mt-3 pt-3 border-t border-blue-200 grid grid-cols-1 gap-2 text-xs">
          {strength.length > 0 && (
            <p className="text-green-700"><span className="font-medium">Strength:</span> {strength.join('; ')}</p>
          )}
          {weakness.length > 0 && (
            <p className="text-red-700"><span className="font-medium">Weakness:</span> {weakness.join('; ')}</p>
          )}
          {risk.length > 0 && (
            <p className="text-yellow-800"><span className="font-medium">Risk:</span> {risk.slice(0, 3).join('; ')}</p>
          )}
        </div>
      )}

      {intelligence.not_a_sell_signal && (
        <p className="mt-3 text-[11px] text-gray-600">Decision support only. Use with your setup, experience, and risk rules.</p>
      )}
    </div>
  );
}
...
function PredictionCard({ prediction }: PredictionCardProps) {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-lg p-4 space-y-3">
      ...
      <MarketIntelligencePanel prediction={prediction} />
